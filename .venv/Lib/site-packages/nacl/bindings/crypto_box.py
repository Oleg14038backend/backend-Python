...
def crypto_box_open_easy(
    ...
    _clen = len(ciphertext)
        ...
        "Input ciphertext must be at least {} long".format(crypto_box_MACBYTES),
...
def crypto_box_open_easy_afternm(ciphertext: bytes, nonce: bytes, k: bytes) -> bytes:
    ...
    res = lib.crypto_box_open_easy_afternm(plaintext, ciphertext, _clen, nonce, k)
    ensure(
        res == 0,
        "An error occurred trying to decrypt the message",
        raising=exc.CryptoError,
...
def crypto_box_seal_open(ciphertext: bytes, pk: bytes, sk: bytes) -> bytes:
    ...
    _clen = len(ciphertext)
        ...
        ("Input ciphertext must be at least {} long").format(crypto_box_SEALBYTES),
        raising=exc.TypeError,
    )
